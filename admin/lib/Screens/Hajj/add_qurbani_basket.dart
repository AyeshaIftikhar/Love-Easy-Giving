...
class AddQurbaniBasket extends StatefulWidget {
  const AddQurbaniBasket({Key? key}) : super(key: key);

  @override
  State<AddQurbaniBasket> createState() => _AddQurbaniBasketState();
}

class _AddQurbaniBasketState extends State<AddQurbaniBasket> {
  final MyAppController _app = Get.find();
  var addQurbaniBasket = GlobalKey<FormState>();
  ...
  TextEditingController basketPrice = TextEditingController();
    ...
    try {
      if (_image != null) {
        ...
        Reference _storageRef =
            storage.ref().child('HajjCharities/${UniqueKey().toString()}');
                  ...
                  child: Column(
                    children: [
                      ...
                      TextFormField(
                        ...
                        validator: (val) {
                          if (val!.isEmpty) {
                            return 'Basket price is required';
                          }
                          return null;
                        },
                        decoration: inputDecoration(
                          hint: 'Basket Price',
                          helper: 'Default basket price will be dealt in Pounds',
                          focusColor: const Color(0xffB08850),
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          child: const TextWidget(
                            text: 'Add Qurbani Basket',
                            size: 17,
                            alignment: Alignment.center,
                            color: Colors.white,
                          ),
                          onPressed: () {
                            if (addQurbaniBasket.currentState!.validate()) {
                              _app.charityRef.add({
                                'Name': name.text,
                                'cName': name.text,
                                ...
                                'image': image.text,
                                'basketPrice': basketPrice.text,
                                'type': 'Sadqah',
                                'Time': DateTime.now(),
                              }).then((value) {
                                Get.snackbar('Basket added successfully', '');
                                // Get.back();
                                Get.offAllNamed('/LoveHajj');
                              });
                              addQurbaniBasket.currentState!.save();
                            }
                          },
